...
class PaymentProcessController
{
    ...
    public function process(): void
    {
        ...
        try {
            ...
            $this->pdo->exec("CREATE TABLE IF NOT EXISTS commandes (
            ...
            $this->pdo->exec("CREATE TABLE IF NOT EXISTS commande_articles (
                id INT PRIMARY KEY AUTO_INCREMENT,
            ...
            $cardholder = preg_replace('/[^a-zA-Zéèêàâôûç\s\-\']/', '', $_POST['cardholder'] ?? '');
            ...
            */
            ...
            try {
                // Créer la commande
                $stmt = $this->pdo->prepare("INSERT INTO commandes (montant_total, statut, email, cree_a)
                                                VALUES (?, 'payee', ?, NOW())");
                $stmt->execute([
                    $payment['amount'],
                ...
                foreach ($payment['items'] as $id => $quantite) {
                    ...
                    // Récupérer le prix
                    $stmt_prix = $this->pdo->prepare("SELECT prix FROM catalogue_funeraire WHERE id = ? LIMIT 1");
                    $stmt_prix->execute([(int) $id]);
                    $produit = $stmt_prix->fetch(PDO::FETCH_ASSOC);

                    if ($produit) {
                        $stmt_insert = $this->pdo->prepare("INSERT INTO commande_articles (commande_id, produit_id, quantite, prix_unitaire)
                                                             VALUES (?, ?, ?, ?)");
                        $stmt_insert->execute([
                            $commande_id,
                            (int) $id,
                ...
                $stmt_stock = $this->pdo->prepare("UPDATE catalogue_funeraire SET stock = stock - ? WHERE id = ? AND stock >= ?");
